#include "sprite.h"
extern int g_iFrameDelay;

Sprite::Sprite()
{
	m_pBitmap=NULL;
	SetRect(&m_rcPosition,0,0,0,0);
...
Sprite::Sprite(Bitmap* pBitmap,int x,int y)
{
	m_pBitmap=pBitmap;
...
SPRITEACTION Sprite::Update()
{
	...
	else if(m_baBoundsAction==BA_STOP)
	{
		if((ptNewPosition.x<m_rcBounds.left)||
			(ptNewPosition.x+ptSpriteSize.x>m_rcBounds.right))
		{
		...
		if((ptNewPosition.y<m_rcBounds.top)||
			(ptNewPosition.y+ptSpriteSize.y>m_rcBounds.bottom))
		{
	...
	else if(m_baBoundsAction==BA_DIE)//die
	{
		if((ptNewPosition.x+ptSpriteSize.x<m_rcBounds.left)||//?
			(ptNewPosition.x>m_rcBounds.right)||
