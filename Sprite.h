...
public:
	RECT m_rcMouseNotify;
	SpriteType m_stType;
	Sprite();
	Sprite(RECT& rcPosition);
	Sprite(Bitmap* pBitmap,int x,int y);
	Sprite(Bitmap* pBitmap,BOUNDSACTION baBoundsAction=BA_NONE);
	Sprite(Bitmap* pBitmap,RECT& rcBounds,
		BOUNDSACTION baBoundsAction=BA_STOP);
	Sprite(Bitmap* pBitmap,POINT ptPostion,POINT ptVelocity,int iZOrder,
	...
	void SetVelocity(POINT ptVelecity);
	...
	void SetFrameDelay(int iFrameDelay){m_iFrameTrigger=m_iFrameDelay=iFrameDelay;}
...
{
	POINT ptPoint;
	ptPoint.x=x;ptPoint.y=y;
	return PtInRect(&m_rcMouseNotify,ptPoint);
}
